...
def gerar_mapa_ateg_consolidado():
    try:
        ...
        folium.GeoJson("https://raw.githubusercontent.com/tbrugz/geodata-br/master/geojson/geojs-23-mun.json", 
        ...
        contagem_prj = df_final.groupby('projeto')['tecnico'].nunique().to_dict()

        # Adicionado "P" para Projetos no dicionário de grupos
        grupos_dict = {"S": {}, "R": {}, "A": {}, "P": {}}
        ...
        for _, row in df_final.iterrows():
            ...
            label_p = f"P: {row['projeto']} ({contagem_prj[row['projeto']]})"

            # Loop de criação dos grupos incluindo a nova aba P (Projetos)
            for tipo, label, mostrar in [("S", label_s, True), (tipo_r, label_r, False), ("A", label_a, False), ("P", label_p, False)]:
                if label not in grupos_dict[tipo]:
                    grupos_dict[tipo][label] = folium.FeatureGroup(name=label, show=mostrar).add_to(m)
                
                marker = folium.Marker([lat_f, lon_f], icon=criar_marcador_div(row['tecnico'], cor_hex, row['tempo_projeto_meses']))
        ...
        document.addEventListener('DOMContentLoaded', function () {
            var observer = new MutationObserver(function () {
                ...
                if (list && !list.querySelector('.btn-mapa')) {
                    ...
                    var dPrj = document.createElement('details'); // NOVA ABA PROJETOS
                    dPrj.innerHTML = "<summary>📂 PROJETOS</summary><div class='lista-interna'></div>";
                    ...
                    labels.forEach(l => {
                        ...
                        else if (txt.startsWith('P:')) dPrj.querySelector('.lista-interna').appendChild(l); // FILTRO PROJETOS
                        else dGeo.querySelector('.lista-interna').appendChild(l);
                    });
                    list.appendChild(dGeo); list.appendChild(dSup); list.appendChild(dReg); list.appendChild(dAtv); list.appendChild(dPrj);
                }
            });
        ...
        m.save("index.html")
        print("✅ Mapa atualizado: Aba PROJETOS incluída!")
