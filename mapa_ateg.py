...
def gerar_mapa_ateg_consolidado():
    try:
        query_sql = """
        SELECT supervisor_atual, tecnico, atividade, municipios, regiao_faec, tempo_projeto_meses, codigos_ibge, data_ultima_visita
        FROM public.vw_tecnicos_atuantes_vs_vinculos
        WHERE TO_DATE(data_ultima_visita, 'DD/MM/YYYY') >= (CURRENT_DATE - INTERVAL '2 months') 
        AND tempo_projeto_meses <= 24 AND total_propriedades_ativas > 0
        ORDER BY supervisor_atual ASC, tecnico ASC
        """
        ...
        # ── ADICIONANDO DIVISÓRIAS MUNICIPAIS (CEARÁ) ──
        ...
        for _, row in df_final.iterrows():
            ...
            if val_reg in REGIOES_OFICIAIS:
                label_r, tipo_r = f"R: {val_reg} ({contagem_reg[val_reg]}){formatar_subtotais(df_final, 'regiao_faec', val_reg)}", "R"
            else:
                label_r, tipo_r = f"A: {val_reg} ({contagem_reg[val_reg]})", "A"
            ...
            for tipo, label, mostrar in [("S", label_s, True), (tipo_r, label_r, False), ("A", label_a, False)]:
                ...
                folium.Marker([lat_f, lon_f], icon=criar_marcador_div(row['tecnico'], cor_hex, row['tempo_projeto_meses']), tooltip=row['tecnico']).add_to(grupos_dict[tipo][label])
        ...
        document.addEventListener('DOMContentLoaded', function () {
            var observer = new MutationObserver(function () {
                ...
                if (list && !list.querySelector('.btn-mapa')) {
                    ...
                    list.appendChild(dGeo);
                    list.appendChild(dSup);
                    list.appendChild(dReg);
                    list.appendChild(dAtv);
                }
            });
        ...
        m.save("index.html")
        print("✅ Mapa com divisórias e otimizado!")
